...
export class RemoveUserFromTaskUseCase {
	...
	async execute({
		...
		userIdToRemove,
		requestingUserRole,
	}: RemoveUserFromTaskUseCaseRequest): Promise<void> {
		if (requestingUserRole !== "ADMIN") {
			throw new Error(
				"Não autorizado: Apenas administradores podem remover atribuições.",
			);
		}

		const isAssigned = await this.taskUserRepository.findByTaskAndUser(
			taskId,
			userIdToRemove,
		);
