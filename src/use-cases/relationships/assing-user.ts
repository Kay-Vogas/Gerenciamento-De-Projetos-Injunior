...
export class AssignUserToTaskUseCase {
    ...
    async execute({ taskId, userIdToAssign, requestingUserRole }: AssignUserToTaskUseCaseRequest): Promise<void> {
        if (requestingUserRole !== 'ADMIN') {
            throw new Error('Não autorizado: Apenas administradores podem atribuir tarefas.');
        }

        const alreadyAssigned = await this.taskUserRepository.findByTaskAndUser(taskId, userIdToAssign);
